        ...
        for brec in individualRecordings:
            ...
            if ep_a != 'NULL' and ep_a != 'None' and ep_b != 'None' and season_a != 'None' and season_b != 'None' and ep_b != 'NULL' and season_a != 'NULL' and season_b != 'NULL' and ep_a == ep_b and season_a == season_b and time_b != time_a and show_a == show_b and state_b == state_a and seriesID_b == seriesID_a:
                ...
                if timeDelta(time_a, time_now) < 0 and timeDelta(time_b, time_now) < 0:
                    print(
                        "Second Recording should not have happened on this account with show " + show_a + " at  " + time_a + " vs " + time_b + " for " + accountName)
...
def convertToString(seriesExtID, programGeneric, accountName, showName, programDetailsID):
    ...
    if hexx.find('EP0') == 0 and programGeneric == True:  # Then we have found that hte ID matches EP
        print(
            hexx + " That should have been programGeneric, but isn't " + accountName + " for " + showName + " " + programDetailsID)
        return 1
        ...
        for eachRec in recs:
            ...
            if (recGroup != None):
                for eachRecGroup in recGroup:
                    ...
                    print(" Series ID " + seriesID + " and the series external ID " + seriesExtID)
                    ...
                    if seriesObjState == "Scheduled":
                        ...
                        if timeDelta(startUTC, time_now) < 0:
                            try:
                                sanityData.setfake_generic(
                                    convertToString(programDetailsID, programDetailsGeneric, accountName, showName,
                                                    programDetailsGLF))
                            except:
                                pass
...
def processResults(sanityData, featureGroupLen):
    ...
    printTestCase(
        "Test Case 2: Number of Occurences of recordings that were scheduled and duplicated with the same episode [MFR-4380]",
        valIndRecs_program)
    js.append({
                  "Test Case 2: Number of Occurences of recordings that were scheduled and duplicated with the same episode [MFR-4380]": valIndRecs_program})
    ...
    printTestCase(
        "Test Case 4: Number of Occurences of invalid account profile configurations (Ingress or Egress is not set) [MFR-9409]",
        valAccountConfiguration_program)
    js.append({
                  "Test Case 4: Number of Occurences of invalid account profile configurations (Ingress or Egress is not set) [MFR-9409]": valAccountConfiguration_program})
    ...
    printTestCase(
        "Test Case 6: Number of Occurences of out-of-sync accounts between DVR Proxy and OSS definitions [MFR-8249]",
        outOfSync)
    js.append({
                  "Test Case 6: Number of Occurences of out-of-sync accounts between DVR Proxy and OSS definitions [MFR-8249]": progDetails})
    ...
    printTestCase("Test Case 13: Number of occurences where DVR station ID did not match OSS station ID",
                  outOfSyncDVRData)
    js.append(
        {"Test Case 13: Number of occurences where DVR station ID did not match OSS station ID": outOfSyncDVRData})
    return js
...
def checkDVREmpty(DVRRECS, OSSRecs, sanityData, accountName):
    for eachDVR in DVRRECS:
        ...
        if dvrShowName == "NULL" or dvrShowName == None or dvrShowName == "" or dvrShowName == "* ERROR * NULL Recordings Object" and re.match(
                '8455', accountName):
            print("This show has empty program detail information (DVR) " + accountName)
            sanityData.setDVREmptyData(1)
        if dvrProgId == None or dvrProgId == "NULL" or dvrProgId == "" or dvrProgId == 'None' and re.match('8455',
                                                                                                           accountName):
            ...
            for eachOSS in OSSRecs:
                if dvr_id == eachOSS['recordingID']:
                    ...
                    break
            try:
                print("This Show is Empty " + showName + " " + startTime + " on channel " + str(
                    channelNumber) + " on " + accountName)
            except:
                pass
        ...
        for eachOSS in OSSRecs:
            ...
            for eachDVR in DVRRECS:
                ...
                if dvr_id == OSS_id:
                    ...
                    flag = True
                    if eachDVR['glfStationID'] != eachOSS['glfStationID'] and eachDVR['glfStationID'] != '' and eachOSS[
                        'glfStationID'] != '':
                        print("Mismatch between GLF station IDs " + eachDVR['glfStationID'] + " " + eachOSS[
                            'glfStationID'])
