...
def get_token(_env, _proxyurl):
    ...
    if _env == 'proda':
        ...
        instance = 'prodc.bce'
        cert = 'credentials/BellVIC_MFSTS_API_SSL.pfx'
        cert_pass = 'BellMF1234'
        token_uri = 'https://appgw-operatorstsssl.' + instance + '.tv3cloud.com/certactive'
        token_filename = './' + instance + '_token.json'
    elif _env == 'prodb':
        instance = 'prodb.bce'
        cert = 'credentials/ossbssTools-prodb-sts.pfx'
        cert_pass = 'Medi@F1rst'
        token_uri = 'https://appgw-operatorstsssl.' + instance + '.tv3cloud.com/certactive'
        token_filename = './' + instance + '_token.json'
    token_valid = 0
        ...
        with open(token_filename, 'r') as token_file_in:
            try:
                ...
                # print(bcolors.OKGREEN + 'token loaded ok. proceeding...\n' + bcolors.ENDC)
                    ...
                    print(bcolors.WARNING + 'token expired. getting new token...\n' + bcolors.ENDC + '\n')
            except:
                print(bcolors.WARNING + 'damaged token file. getting new token...' + bcolors.ENDC + '\n')
    except OSError:
        print(bcolors.WARNING + 'no token file. getting new token...' + bcolors.ENDC + '\n')
    if (token_valid):
        pass
    else:
        ...
        if (response.status_code == 200):
            ...
            print(bcolors.WARNING + 'token request response: ', response.json()['ErrorDescription'],
                  bcolors.ENDC + '\n')
            sys.exit(bcolors.FAIL + 'invalid cert or wrong password' + bcolors.ENDC + '\n')
    try:
...
def get_mf_image_status(_url):
    response = session.head(_url)
    code = response.status_code
    return code


# provided_call_sign = "CTVHT"
    ...
    while True:
        ...
        for doc in mytab.find({"mf_uid": "*" + str(seq_number)}).limit(50):
            ...
            for size in res_images:
                count_images += 1
                # code = -1
                code = get_mf_image_status(res_images[size])
                if int(code) != int(200):
                    count_fails += 1
                    tally_failed += 1
            if (count_images == 0):
...
if __name__ == '__main__':
    ...
    try:
        ...
        if past_ver != None:
            get_delta = True
            print("Using deltas - past version: " + str(past_ver))
        else:
            get_delta = False
            print("Not using deltas")

    out_file_name = date_str + "_" + env + "_output.txt"
    m = connect_mongo(date_str, env)
    mytab = m['connection']
    mongo_connected = m['Mongo_Connected']
    mongo_count = mytab.count_documents({"mf_k119": None})
    epg_file = "epg.xml"
    if env == "prodb":
        epg_file = "epg-prodb.xml"
    elif env == "prodc":
        epg_file = "epg-prodc.xml"
    if mongo_count == 0:
                ...
                for el in program.iter():
                    ...
                    if el.tag == 'k':
                        ...
                        if (id == "8"):
                            ...
                            k119 = str(el.attrib['v'])
                        elif (id == "2"):
                            k2 = str(el.attrib['v'])
                        elif (id == "100"):
                            k100 = str(el.attrib['v'])
                        elif (id == "121"):
                            k121 = str(el.attrib['v'])
                pid = int(program.attrib['id'])
                if (mongo_connected):
                    res = {
                        "pid": pid,
                        "glf_title": title,
                        "glf_episode_title": episode_title,
                        "glf_description": d,
                        "glf_oad": oad,
                        "glf_rating": k100,
                        "glf_language": lang,
                        ...
                        "glf_k2": str(k2),
                        "glf_k121": k121,
                        "mf_uid": "*" + str(seq_count),
                    }
                    seq_count += 1
                    if seq_count == seq_max:
                        seq_count = 0
                    ret = mytab.insert_one(res)
                else:
                    print("Mongo not connected!")
    else:
        print("Already ingested " + str(mongo_count) + " documents - proceeding with matching.")
    print("Completed ingesting GLF into MongoDB - matching commencing.")
    pool = mp.Pool(seq_max)
    slices = seq_max
    for i in range(0, slices):
        pool.apply_async(mf_map_program_parallel, args=(i, tok, date_str, env, past_ver))
        # mf_map_program_parallel(i, tok, date_str, env, past_ver)
    while True:
        count_remaining = mytab.count_documents({"mf_k119": None})
        if (count_remaining == 0):
            break
        print(str(datetime.datetime.now()) + " | " + str(count_remaining) + " remaining.")
        time.sleep(30)
